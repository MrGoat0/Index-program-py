...
def es_pdf(pdf):  #Esta función lee un .pdf o .txt y le saca un indice
    ...
    if ('.PDF' in pdf) or ('.pdf' in pdf):  # Si es .pdf va aqui
        ...
        while i<pdfread.getNumPages():
            ...
            for j in x:
                word=limpiador(quitar_puntuación(j)) #llama función quitar puntuación y limpiador
                if word not in d:
                    d[word]=[]
                    d[word].append(1)
                else:
        ...
        with open(pdf) as line:   # Si es .txt va aqui 
            ...
            for j in lines:
                x=j.split()
                for k in x:
                    word=limpiador(quitar_puntuación(k)) #llama función quitar puntuación y limpiador
                    if word not in d:
                        d[word]=[]
                        d[word].append(1)
                    else:
...
def crear_indice(ruta): #Esta función crea un indice de la ruta seleccionada y devuelve una lista para buscar 
    ...
    arch=1
    for i in d:
        #'palabra': [[1(num pdf),1(repeticiones palabra)],[1(num pdf),2(repeticiones palabra)]]. Este es el formato dentro del indice, al tratarse de un diccionario, será rápido buscar con un for
        for word in i:        
...
def crear_txt_titulos():  #Crea un .txt con los titulos de los archivos para añadirlo al indice inverso
    global ruta
    contenido=os.listdir(ruta)
...
def cargar_index(ruta):
    contenido=os.listdir(ruta)
    if 'index.pickle' in contenido: #Carga un 'index.pickle' que ya existe
        menu_main2()       
    else:
        if 'titulos' not in contenido:
            crear_txt_titulos()
            crear_indice(ruta)
            menu_main2() #Si no encuentra el 'index.pickle' en la ruta llama a la función para crear un indice
...
def abrir_archivos():  #Esta función muestra al usuario los posibles resultados y el número para abrir el archivo,
    ...
    print('| Ruta del archivo ||| Número para abrir el archivo |')
...
def hacer_busqueda(): 
    ...
    posible_pdf=[]
    contenido=os.listdir(ruta)
    term=input('¿Qué quieres buscar?: ')
    if term.isalpha()==True: #confirma que la entrada sirva para buscar
        term_ready=stemming(quitar_puntuación(term.lower())) #limpia la palabra ingresada
        x=open(ruta+'/index.pickle','rb')
        indice=pickle.load(x)
        for i in indice:
            if i == term_ready:
                primera_busqueda.append(indice.get(i))
        print('Tienes las siguientes opciones: \n')   
        for i in primera_busqueda:
            for j in i:
        ...
        if len(primera_busqueda)>0:
            abrir_archivos()
        else:
            menu_main2()
    else:
        hacer_busqueda() #vuelve a la función 

####################################################################################################################################################
def menu_busqueda1(): #Crea el index del directorio colocado y devuelve al siguiente menu donde se realiza la busqueda
    global ruta
    print()
    print('_Tu directorio debe contener '"(letra mayúscula)"':/ para ser valido.\n')
...
def menu_main2(): #Este menu muestra la opción de hacer una busqueda
    ...
    'Créditos: 3'+'\n'+
    'Salir del motor de busqueda: qx2')
    x=str(input())
    if x == 'q':
        salir=1
        return
